...
class BaseModel():
	...
	def get_build_graph_fn(config, mode):
		pass

	def train(logdir='log/', save_checkpoint_secs=60, save_summaries_steps=60):
		with tf.Graph().as_default():
		  self.get_build_graph_fn('train')()
...
class BaseConfig():
	def __init__(self, optimizer, learning_rate, steps):
		self.optimizer = optimizer
		self.learning_rate = learning_rate
		self.steps = steps

class LSTMConfig(BaseConfig):
	def __init(self, optimizer, learning_rate, steps, 
		encoder_decoder, rnn_layers, dropouts):
		self.encoder_decoder = encoder_decoder
		self.rnn_layers = rnn_layers
		self.dropouts = dropouts

		super(LSTMConfig, self).__init__(optimizer, learning_rate, steps)
